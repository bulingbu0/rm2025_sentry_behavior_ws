...
{
  int game_progress, min_remain_time, max_remain_time;
  ...
  RCLCPP_INFO(logger_, "gameprogress: %d", game_progress);
  RCLCPP_DEBUG(
    logger_, "Checking: Progress(%d/%d), Remain Time(%ds) in [%d-%d]",
    static_cast<int>(msg->game_progress), game_progress, msg->stage_remain_time,
    min_remain_time, max_remain_time);

  const bool is_progress_match = (msg->game_progress == game_progress);
...
BT::PortsList IsGameStatusCondition::providedPorts()
{
  return {
    ...
    BT::InputPort<int>("game_progress", 4, "Expected game progress stage"),
