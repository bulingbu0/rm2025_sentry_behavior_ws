...
bool CalculateAttackPoseAction::setMessage(visualization_msgs::msg::MarkerArray & msg)
{
  ...
  if (!tracker_target->tracking && tracker_target->id == "") {
    ...
    // Transform enemy position改变敌人位置

    PointStamped enemy_point;
    enemy_point.header = tracker_target->header;
    enemy_point.point = tracker_target->position;
    RCLCPP_INFO(node_->get_logger(), "Tracker target: %.2f, %.2f, %.2f", enemy_point.point.x, enemy_point.point.y, enemy_point.point.z);
    // geometry_msgs::msg::PointStamped initial_point;
    // initial_point.header = tracker_target->header;
    // initial_point.point.x = 0.0;
    // initial_point.point.y = 0.0;
    // initial_point.point.z = 0.3;
    // try {
    // enemy_point = tf_buffer_->transform(initial_point, "base_link", tf2::durationFromSec(params_.transform_tolerance));
    // } catch (tf2::TransformException & ex) {
    //   RCLCPP_ERROR(node_->get_logger(), "Failed to transform enemy position to base_link: %s", ex.what());
    //   return false;
    // }
    // target_frame: global_costmap->header.frame_id
    ...
    RCLCPP_INFO(node_->get_logger(), "Robot pose: %.2f, %.2f", robot_pose.pose.position.x, robot_pose.pose.position.y);
    ...
    RCLCPP_INFO(
      node_->get_logger(), "Attack pose: %.2f, %.2f", attack_pose.pose.position.x,
...
//     return true;
//   } catch (tf2::LookupException & ex) {
//     RCLCPP_ERROR(logger, "No Transform available Error: %s", ex.what());
//   } catch (tf2::ConnectivityException & ex) {
//     RCLCPP_ERROR(logger, "Connectivity Error: %s", ex.what());
//   } catch (tf2::ExtrapolationException & ex) {
//     RCLCPP_ERROR(logger, "Extrapolation Error: %s", ex.what());
//   } catch (tf2::TimeoutException & ex) {
//     RCLCPP_ERROR(logger, "Transform timeout: %.4f seconds", transform_timeout);
//   } catch (tf2::TransformException & ex) {
//     RCLCPP_ERROR(
